...
def naive_attn(q:torch.Tensor, k:torch.Tensor, v:torch.Tensor):
    HEAD_DIM = q.shape[-1]
    output = torch.matmul(torch.softmax(
                        torch.matmul(q, k.transpose(-1, -2))*(HEAD_DIM**-0.5) ), # type: ignore
                    v)

def attn_flash_attn_impl(q, k, v):
   return flash_attn_func(q, k, v)

def main():
    # --- 1. 设置设备 ---
    # 检查是否有 GPU，Triton 通常在 GPU 上运行
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device}")

    # --- 2. 定义模型参数 ---
    # 注意：为了 Triton 的兼容性，序列长度最好能被 block_size 整除（或者代码内部会处理 padding）
    BATCH = 2
    N_HEADS = 8
    SEQ_LEN = 1024# *128 # 序列长度
    HEAD_DIM = 64   # Head dimension
    TOP_K = 64      # 每个 Query block 只关注 Top-K 个 Key/Value blocks

    print(f"Configuration: Batch={BATCH}, Heads={N_HEADS}, SeqLen={SEQ_LEN}, HeadDim={HEAD_DIM}, TopK={TOP_K}")

    # --- 3. 创建随机输入数据 ---
    # 注意：形状必须是 [BATCH, N_HEADS, N_CTX, D_HEAD]
    ...
    # --- 4. 调用稀疏注意力函数 ---
    print("Running block_sparse_attention...")
    
    # PyTorch 为了优化显存，在某些情况下可能会复用显存。为了准确计时，我们需要同步 CUDA。
    if device.type == "cuda":
        torch.cuda.synchronize()

    # 调用函数
    # output = block_sparse_attention(q, k, v, top_k=TOP_K)
    
    # output = naive_attn(q, k, v)

    output = attn_flash_attn_impl(q, k,v )
    ...
    # --- 5. 输出结果 ---
    print(f"Success!")
    print(f"Output shape: {output.shape}") # 应该是 [2, 8, 1024, 64]
    print(f"Output device: {output.device}")
    
    # 简单检查：确保输出不是 NaN
    assert not torch.isnan(output).any(), "Output contains NaN values!"
    print("No NaN values found in output.")
